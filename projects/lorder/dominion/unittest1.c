...
void test_buyCard()
{
	...
	assertResult = unitTestAssert("buyCard should return -1 when numBuys < 1", (result == -1));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	assertResult = unitTestAssert("Game state should not be modified when numBuys < 1", (changed == 0));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	testG.supplyCount[CARDTOBUY] = 2;
  	...
  	//the game state should not have changed
	changed = isGameStateChanged(&G, &testG);
	assertResult = unitTestAssert("Game state should not be modified when coins < cost", (changed == 0));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	assertResult = unitTestAssert("buyCard should return -1 when coins < cost", (result == -1));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	testG.numBuys = 1;
	testG.coins = 20;
	testG.supplyCount[CARDTOBUY] = 0;

  	result = buyCard(CARDTOBUY, &testG);

  	//the game state should not have changed
	changed = isGameStateChanged(&G, &testG);
	assertResult = unitTestAssert("Game state should not be modified when supplyCount < 1", (changed == 0));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
  	...
  	/* Should return -1 when coins < cost */
	assertResult = unitTestAssert("buyCard should return -1 when supplyCount < 1", (result == -1));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	testG.coins = 20;
	testG.supplyCount[CARDTOBUY] = 2;
	testG.deck[Player2][0] = silver;
	testG.deck[Player2][1] = smithy;
	testG.deckCount[Player2] = 2;
	testG.hand[Player2][0] = gardens;
	testG.hand[Player2][1] = council_room;
	testG.handCount[Player2] = 2;

	result = buyCard(CARDTOBUY, &testG);

	/* Should return 0 when a card is purchased */
  	assertResult = unitTestAssert("Should return 0 when a card is purchased", (result == 0));
  	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	/* Should reduce coins by purchased card cost */
  	assertResult = unitTestAssert("Should reduce player's coins by purchased card cost", (testG.coins == G.coins - getCost(CARDTOBUY)));
  	if(assertResult == 0)
	{
		testFailed = 1;
	}
  	...
  	assertResult = unitTestAssert("Should reduce card supplyCount by 1", (testG.supplyCount[CARDTOBUY] == G.supplyCount[CARDTOBUY] - 1));
  	if(assertResult == 0)
	{
		testFailed = 1;
	}
  	...
  	assertResult = unitTestAssert("Should reduce numBuys by 1", (testG.numBuys == G.numBuys - 1));
  	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	/* Should add purchased card to discard pile */
  	assertResult = unitTestAssert("Should add purchased card to discard pile", (hasCard(CARDTOBUY, testG.discard[Player1], testG.discardCount[Player1])));
  	if(assertResult == 0)
	{
		testFailed = 1;
	}
  	...
  	assertResult = unitTestAssert("Should increase discardCount by 1", (testG.discardCount[Player1] == G.discardCount[Player1] + 1));
	if(assertResult == 0)
	{
		testFailed = 1;
	}
