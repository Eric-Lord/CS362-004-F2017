...
int test_Outpost() 
{
  ...
  assertResult = unitTestAssert("Should increment the outpostPlayed flag", (testG.outpostPlayed == G.outpostPlayed + 1));
  if(assertResult == 0)
  {
    testFailed = 1;
  }
  ...
  assertResult = unitTestAssert("Should move played card to playedCards - playedCards should have outpost",
    (hasCard(outpost, testG.playedCards, testG.playedCardCount)));
