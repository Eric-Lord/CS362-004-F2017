...
void test_isGameOver()
{
	...
	assertResult = unitTestAssert("isGameOver should return 0 when supplyCount[province] > 0", (result == 0),2);
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	assertResult = unitTestAssert("isGameOver should return 0 when supplyCount[province] > 0 and only one supply pile is empty", (result == 0),2);
	if(assertResult == 0)
	{
		testFailed = 1;
	}
	...
	assertResult = unitTestAssert("isGameOver should return 0 when supplyCount[province] > 0 and only two supply piles are empty", (result == 0),2);
	if(assertResult == 0)
	{
		testFailed = 1;
	}
